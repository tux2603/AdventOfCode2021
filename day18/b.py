...
def add_lists(list_a, list_b):
    ...
    while True:
        ...
        if pair_start > -1 and pair_end > -1:
            before = combined[:pair_start]
            pair = combined[pair_start:pair_end]
            after = combined[pair_end:]
        ...
        if large_number:
            ...
            # print(f'{combined[:large_number_start]}\033[1m{combined[large_number_start:large_number_end]}\033[0m{combined[large_number_end:]}')

            number = int(combined[large_number_start:large_number_end])
            ...
            combined = combined[:large_number_start] + f'[{half_round_down},{half_round_up}]' + combined[large_number_end:]
...
for i, num1 in enumerate(numbers):
    for j, num2 in enumerate(numbers):
        if i != j:
            combined = add_lists(num1, num2)
            value = compute_magnitude(eval(combined))
            if value > max_magnitude:
                print(f'Found new max: {value} for {num1} + {num2}')
                max_magnitude = value
